...
- OpenAI extension reference: the user-supplied three-card screenshot at `/var/folders/s0/7l_6b47s7v33tf9wc_c0pqtr0000gn/T/TemporaryItems/NSIRD_screencaptureui_Srkd9B/Screenshot 2026-09-22 at 1.11.37 AM.png` (2754 × 638 px). The fourth-card implementation was browser-captured at 1280 × 720 px and 390 × 844 px in the Codex in-app browser; these captures were compared with the reference card region and not persisted to files by the browser tool.
...
The hero and model cards were also inspected separately at full viewport size. GLM, Gemini, Kimi and OpenAI now share the domestic card anatomy and interactive selected state; the code example panel sits beneath them. The four-card desktop comparison preserves equal card widths, gaps, borders, title alignment and option rows. At 390 px, cards stack as full-width rows, the code sample stays below them, and `scrollWidth === innerWidth`. The mobile art crop was shifted right to prevent the wordmark from reducing English-heading contrast.
...
- Spacing and layout: the same container width, 69 px desktop header, hero height, section rhythm, card radii and model/code-panel spacing are reused. Four international model families fill the desktop row; the grid becomes two columns on tablets and one on phones.
...
- Images and assets: the exact domestic dataflow image and Guanqi mark are reused at native resolution. Provider marks from the domestic asset set are used where applicable; the OpenAI Blossom is the unmodified white SVG from OpenAI's official brand download (`https://cdn.openai.com/brand/openai-logos.zip`), displayed at equal apparent icon scale. The existing Hugeicons library supplies remaining UI icons. OpenAI owns its name and Blossom mark.
- Copy and content: international URLs (`api.tokenk8s.com` and `docs.tokenk8s.com`), English-first copy and GLM/Gemini/Kimi/OpenAI listings are intentional. `gpt-5.5` and `gpt-5.4` were present in the site's public pricing list before the card was added. No mainland ICP filing was copied.

## Interaction and technical checks

- Model selection updates the code example; Python, curl and Node.js tabs switch correctly.
- Selecting `gpt-5.5` or `gpt-5.4` updates the example's `model` field; the OpenAI card shows its selected state in both languages.
...
5. OpenAI card pass: [P2] the official Blossom asset's built-in clear space made its visible mark look too small beside other provider marks. Increased only its rendered box to 44 px while leaving the SVG unaltered; the follow-up desktop and mobile captures show comparable apparent icon scale and no card overflow.
